...
def spaghettiplot(XRraw, XRmeta, v1, v2):
        fig = make_subplots(rows=1, cols=2, subplot_titles=("Strong model dependence",
                                                        "Strong climate target dependence"),
                        horizontal_spacing = 0.15, column_widths=[0.33, 0.33])

        # (a) Example model-dependent: Secondary Energy|Electricity|Biomass w/o CCS
        xrsub = XRraw.sel(Variable = v1)
        ...
        ccati = np.array([i[1] for i in ccat]).astype(int)

        for ms in range(len(modscens)):
                mod = mods[ms]
                col = 'silver'
                fig.add_trace(go.Scatter(x=xrsub.Time,
                                        y=values[ms],
                                        marker={'color': col},
                                        line={'width':0.5},
                                        mode='lines',
        ...
        # (b) Example C-dependent: Emissions|CO2
        xrsub = XRraw.sel(Variable = v2)
        xrsub = xrsub.dropna('ModelScenario', how='any')
        values = np.array(xrsub.Value)
        modscens = np.array(xrsub.ModelScenario)
        mods = np.array([i.split('|')[0] for i in modscens])
        ...
        # Lines
        draw_line(fig, 'WITCH',     0.445, 0.20, 0.42, 'forestgreen', 0.46, 0.26)
        draw_line(fig, 'TIAM-ECN',  0.445, 0.43, 0.495, 'purple', 0.46, 0.47)
        draw_line(fig, 'COFFEE',    0.445, 0.62, 0.91, 'steelblue', 0.46, 0.82)
...
>>>>>>> 5c0e54aca78664d6bd1c9757282795ba99d164ef
        ...
        fig.update_yaxes(title=v1+" [EJ/yr]", row=1, col=1)
        fig.update_yaxes(title=v2+" [Mt CO2/yr]", row=1, col=2)
        fig.update_layout(height=600, width = 1200)
        return fig

def drawline_brack(fig, x, y1, y2, color, backwards='n'):
    fig.add_shape(type="line", xref='paper', yref='paper',
                    x0=x,
                    y0=y1,
                    x1=x,
...
def drawline_single(fig, x1, x2, y1, y2, color):
    fig.add_shape(type="line", xref='paper', yref='paper',
                    x0=x1,
                    y0=y1,
                    x1=x2,
